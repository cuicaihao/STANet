...
def val(opt, model):
    ...
    web_dir = os.path.join(opt.checkpoints_dir, opt.name, '%s_%s' % (
    ...
    if opt.display_id > 0:
        visualizer.plot_current_acc(
            epoch, float(epoch_iter) / dataset_size, score)
       ...
       # miou_current = val(opt, model)
        ...
        if epoch % opt.save_epoch_freq == 0:              # cache our model every <save_epoch_freq> epochs
            ...
            if miou_current > miou_best:
                ...
                epoch_best = epoch
                model.save_networks(str(epoch_best)+"_" +
                                    metric_name+'_'+'%0.5f' % miou_best)
